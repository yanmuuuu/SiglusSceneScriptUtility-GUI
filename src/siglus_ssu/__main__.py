...
def _usage(out=None):
    ...
    text = (
        ...
        "    --tmp           Use specific temp directory (not with --debug/--dat-repack/--set-shuffle/--test-shuffle)\n"
        ...
        "\n"
        "Analyze mode:\n"
        f"  {p} -a [--disam] [--readall|--apply] <input_file.(pck|dat|gan|sav|cgm|tcr)>\n"
        f"  {p} -a [--payload] <input_file_1> <input_file_2>\n"
        f"  {p} -a --word <input_pck> [output_csv]\n"
        f"  {p} -a <path_to_\u6697\u53f7.dat|Scene.pck|SiglusEngine.exe|dir|literal_angou> --angou\n"
        ...
        "    --angou        Parse \u6697\u53f7.dat, extract it from .pck, read SiglusEngine.exe, or use a literal string; print derived exe_el key\n"
